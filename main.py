"""
TELEGRAM AUTO IP EXTRACTOR BOT - FLOODWAIT FIXED
Auto handle floodwait + smart delays
"""
...
class AutoIPExtractor:
    def __init__(self):
        ...
        self.scanning = False
        self.running = True
        self.processed_groups = set()  # Track processed groups
        
    async def start(self):
        """Start all clients"""
        # Start bot
        self.bot = Client("bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, workers=2)
        ...
        # Auto scan on startup with delay
        logger.info("⏳ Waiting 30 seconds before initial scan...")
    ...
    async def scan_all_groups(self, status_msg=None, chat_id=None):
        """Scan ALL groups with floodwait handling"""
        if self.scanning:
            return
        ...
        try:
            ...
            for client in self.user_clients:
                try:
                    async for dialog in client.get_dialogs(limit=50):  # Reduced limit
            ...
            for group in unique_groups:
                ...
                try:
                    ...
                    if group["chat_id"] in self.processed_groups:
                        continue
                    ...
                    if call:
                        ...
                        if ips:
                            ...
                            self.processed_groups.add(group["chat_id"])
                            total_ips += len(ips)
                    ...
                    if status_msg:
                        await status_msg.edit(f"⏳ Rate limited! Waiting {wait_time}s...")
    ...
    async def get_call(self, client, peer):
        """Get active call object with floodwait handling"""
        try:
            ...
            logger.warning(f"⏳ FloodWait in get_call: {e.value}s")
            raise
        except:
            return None
    
    async def extract_ips_from_call(self, client, chat_id, call):
        """Extract IPs from call - automatic join with floodwait handling"""
        ips = []
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⏳ FloodWait on join: {e.value}s")
                raise
            except Exception as e:
        ...
        return ips[:10]  # Max 10 IPs per group
